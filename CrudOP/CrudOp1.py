...
def delete():
    if(e_id.get()==""):
        MessageBox.showinfo("Delete Status","ID is complusary for delete")
    else:
        con=mysql.connect(host="localhost",user="root",password="",database="python-tkinter")
        cursor=con.cursor()
...
def get():
    if(e_id.get()==""):
        MessageBox.showinfo("Fetch Status","ID is complusary for delete")
    else:
        con=mysql.connect(host="localhost",user="root",password="",database="python-tkinter")
        cursor=con.cursor()
...
id=Label(root,text="Welcome to Student Forum",font={'bold',50},bg="white")
id.place(x=200,y=5)
...
phone=Label(root,text="Enter Phone No",font={'bold',10})
...
update.place(x=150,y=140)

get=Button(root,text="Get",font={"italic 10"},bg="white",command=get)
get.place(x=225,y=140)

list=Listbox(root,bg="white")
